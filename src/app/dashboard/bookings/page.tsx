...
export default function MyBookingsPage() {
  ...
  const countOf = (s: BookingStatus) =>
    all.filter((b) => b.status === s).length;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {[
          ...
          <div
            key={label}
            className={`flex items-center gap-3 rounded-2xl border border-[var(--border)] ${bg} p-4`}
          >
            ...
            <div>
              ...
              <p className={`mt-0.5 text-lg font-bold ${color}`}>
                {loading ? (
                  <span className="inline-block h-6 w-10 animate-pulse rounded-lg bg-gray-200" />
                ) : (
                  value
                )}
              </p>
