...
const DOC_CATALOGUE: DocConfig[] = [
  {
    ...
    description:
      'Valid driving license issued by the government. Must be current and not expired.',
    icon: CreditCard,
    required: true,
  ...
  },
  {
    ...
    description:
      "Registration certificate of the vehicle you'll use for rides.",
    icon: Car,
    required: true,
    acceptedFormats: 'JPG, PNG, PDF',
    maxSizeMB: 5,
...
function DocumentCard({
  ...
  return (
    <div
      className={`rounded-2xl border ${sd.border} bg-white p-5 transition-all hover:shadow-md`}
    >
      <div className="flex items-start justify-between gap-3">
        {/* Icon + info */}
        <div className="flex items-start gap-3">
          <div
            className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${status === 'VERIFIED' ? 'bg-green-50' : status === 'REJECTED' ? 'bg-red-50' : 'bg-[var(--primary-light)]'}`}
          >
            <DocIcon
              size={20}
              className={
        ...
        {/* Status chip */}
        <span
          className={`flex shrink-0 items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-semibold ${sd.chip}`}
        >
          <StatusIcon size={11} />
          {sd.label}
        </span>
      ...
      {status === 'REJECTED' && (
        <div className="mt-3 flex items-start gap-2 rounded-xl bg-red-50 p-3 text-xs text-red-600">
          ...
          <span>
            Document was rejected. Please re-upload a clearer copy with all
            details visible.
          </span>
        </div>
      )}
        ...
        <div className="mt-4">
          {localFile && uploading ? (
            ...
            <div className="flex items-center justify-between rounded-xl border border-green-200 bg-green-50 p-3">
              ...
              <span className="ml-2 shrink-0 text-[10px] font-medium text-green-600">
                Uploaded
              </span>
            </div>
          ) : (
...
function VerificationBanner({ items }: { items: VerificationItem[] }) {
  ...
  const verified = items.filter(
    (i) =>
      i.status === 'VERIFIED' &&
      DOC_CATALOGUE.find((d) => d.documentType === i.documentType),
  ).length;
  ...
  return (
    <div
      className={`rounded-2xl p-5 ${allDone ? 'border border-green-200 bg-green-50' : 'border border-[var(--border)] bg-white'}`}
    >
      <div className="flex items-center gap-3">
        <div
          className={`flex h-10 w-10 items-center justify-center rounded-full ${allDone ? 'bg-green-100' : 'bg-[var(--primary-light)]'}`}
        >
          <ShieldCheck
            size={20}
            className={allDone ? 'text-green-600' : 'text-[var(--primary)]'}
          />
        </div>
