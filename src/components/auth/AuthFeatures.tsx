...
export default function AuthFeatures() {
  return (
    <div className="hidden grid-cols-4 overflow-hidden rounded-2xl bg-black/70 shadow-lg backdrop-blur-lg lg:grid">
      {FEATURES.map(({ icon: Icon, title, description }, index) => (
        ...
        >
          <Icon size={22} className="mb-2 text-[var(--primary)]" />

          <h4 className="text-xs font-semibold text-white">{title}</h4>

          <p className="mt-1 max-w-[140px] text-[11px] leading-4 text-white/70">
            {description}
          </p>
        </div>
      ))}
